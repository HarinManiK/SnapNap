        ...
        with self._lock:
            ...
            try:
                ...
                if win32gui.IsWindow(hwnd):
                    # Post restore via the window's own message queue
...
def _make_icon_image(size: int = 64) -> Image.Image:
    ...
    blue  = "#3B3BF5"
    black = "#000000"
    draw.rectangle([0,    0,    half, half], fill=black)   # top-left
    draw.rectangle([half, 0,    size, half], fill=blue)    # top-right
    draw.rectangle([0,    half, half, size], fill=blue)    # bottom-left
    draw.rectangle([half, half, size, size], fill=black)   # bottom-right
    return img
